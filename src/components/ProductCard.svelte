...
<div class="grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-14 lg:gap-20 relative text-base mb-12">
	<div class={'flex flex-col gap-8 md:gap-10 pt-8 sm-pt-10 items-start' + (index % 2 === 1 ? ' md:order-2' : '')}>
		<h4
		...
		<div v class="flex flex-col gap-3">
			{#each productFeature.featureList ?? [] as listItem}
				<div class="flex gap-2 items-center">
					...
					>
						<i class="fa-solid fa-bold text-green-400"></i>
					</div>
					<p>{listItem}</p>
				</div>
			{/each}
